...
-- Calculando la diferencia entre fecha de retorno y de alquiler
SELECT
    DISTINCT f.title,
    f.rental_duration,
    AVG(DATEDIFF(r.return_date, r.rental_date)) OVER (PARTITION BY f.film_id) AS avg_rental_duration
FROM
    film f
JOIN
...
-- Usando solo el rental duration y la particion
SELECT
film_id,
title,
AVG(rental_duration) OVER (PARTITION BY film_id) AS avg_rental_duration
FROM
    film;
...
SELECT
    ...
    rental_date,
    amount,
    SUM(amount) OVER (PARTITION BY DATE(rental_date) ORDER BY rental_date) AS daily_revenue
FROM (
    SELECT
